...
public class CountIndicatorView extends View {
	...
	public CountIndicatorView(Context context, AttributeSet attributeSet) {
		...
		TypedArray styledAttributes = getContext().obtainStyledAttributes(
				attributeSet, R.styleable.CountIndicatorView);
		radius = styledAttributes.getInt(R.styleable.CountIndicatorView_radius,
				radius);
		numberOfItems = styledAttributes.getInt(
				R.styleable.CountIndicatorView_numberOfItems, numberOfItems);
		numberSelected = styledAttributes.getInt(
				R.styleable.CountIndicatorView_numberSelected, numberSelected);
		itemSpacing = styledAttributes.getInt(
				R.styleable.CountIndicatorView_itemSpacing, itemSpacing);
		selectedColour = styledAttributes.getColor(
				R.styleable.CountIndicatorView_selectedColor, selectedColour);
		unSelectedColour = styledAttributes.getColor(
				R.styleable.CountIndicatorView_unSelectedColor,
				unSelectedColour);
	...
	protected void onDraw(Canvas canvas) {
		...
		for (int i = 0; i < numberOfItems; i++) {
			float x = i * itemSpacing + (i * (radius * 2)) + radius
					+ getPaddingLeft();
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		super.onMeasure(widthMeasureSpec, heightMeasureSpec); // To change body
																// of overridden
																// methods use
																// File |
																// Settings |
																// File
																// Templates.
		int width = (numberOfItems * (radius * 2))
				+ ((numberOfItems - 1) * itemSpacing) + getPaddingLeft()
				+ getPaddingRight();
	...
	private Paint getPaint(int i) {
		...
		p.setColor(getResources().getColor(selectedColour));
		return p;
	}
}